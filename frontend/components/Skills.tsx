...
export const Skills: React.FC = () => {
  // Flatten and duplicate skills for marquee effect
  ...
  const MarqueeRow = ({ items, reverse = false }: { items: string[], reverse?: boolean }) => (
    <div className="flex overflow-hidden whitespace-nowrap w-full relative py-4 mask-edges">
      <div className={`flex gap-4 w-max ${reverse ? 'animate-marquee-reverse' : 'animate-marquee'}`}>
        {/* Render 3 sets to ensure seamless infinite scroll on wide screens */}
        {[...Array(3)].map((_, i) => (
          <div key={i} className="flex gap-4">
            {items.map((item, j) => (
              <div 
                key={`${i}-${j}`}
                className="px-6 py-3 text-lg font-mono rounded-full bg-zinc-900/50 border border-white/5 text-zinc-400 hover:text-accent hover:border-accent/50 hover:bg-accent/10 hover:shadow-[0_0_20px_rgba(16,185,129,0.2)] transition-all duration-300 cursor-default whitespace-nowrap"
              >
                {item}
              </div>
            ))}
